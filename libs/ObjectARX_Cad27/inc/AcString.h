...
class AcString : public AcHeapOperators
{
  public:
    ...
    explicit AcString(wchar_t wch) : AcString()
    {
...
#define ENABLE_ACSTRING_CSTRING_OPERATORS
    ...
    AcString(std::wstring_view str) : AcString( str.data(), (Adesk::UInt32)str.length() )
    {
    ...
    /// </summary>
    /// <param name="pszDelims">A string containing token delimiters.
    ...
    ///           Consecutive delimiters will be skipped
    ///           Empty or null pszDelims will return remaining string from iStart
    ///           Clients processing in a loop can use either empty return string
    ///           or iStart == -1 to end tokenization</remarks>

    AcString tokenize(const wchar_t* pszDelims, Adesk::Int32& iStart) const
    {
        AcStrLOG(this,0,L"tokenize: \"%ls\", %d",pszDelims?pszDelims:L"(nullptr)", iStart);
        AcStrASSERT(DBGverify());
        AcStrASSERT(!isGetBuffer());
        ...
        if (ustart < len) // valid start (rejects negative iStart)
        {
            ...
            else
            {
                ...
                for (; i < len; )
                {
                    ...
                    for (;(i < len) && !isOneOf(src[i],pszDelims);i++);
    ...
    /// <summary> Get the last character of the string. </summary>
    /// <returns> The last char of the string, or a null terminator if it's empty. </returns>
    /// <remarks> Similar to std::string::back(), but can be called on an empty string.
    ///           Also unlike back(), the char is returned by value, not by reference.
    /// </remarks>
    wchar_t getLast() const
    {
        return this->isEmpty() ? L'\0' : this->getAt(this->iGetLen() - 1);
    }
