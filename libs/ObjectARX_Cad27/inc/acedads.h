...
ACCORE_PORT int     acedDefun   (const ACHAR *pszName, int nFuncNum);
ACCORE_PORT int     acedDefunEx (const ACHAR *pszGlobalName,
                     const ACHAR *pszLocalName, int nFuncNum);

/* Define help for that external subroutine */
ACCORE_PORT int            acedSetFunHelp (const ACHAR* pszFunctionName,
                               const ACHAR* pszHelpfile,
                               const ACHAR* pszTopic, int iCmd);

/* Undefine an external subroutine in AutoLISP */
ACCORE_PORT int            acedUndef (const ACHAR *sname, int nFuncNum);

// Utilities for external subroutine interface
ACCORE_PORT int            acedGetFunCode (void);
ACCORE_PORT struct resbuf *acedGetArgs (void);

/* Return the specified type as a result of external subroutine */
ACCORE_PORT int            acedRetList (const struct resbuf *rbuf);
ACCORE_PORT int            acedRetVal (const struct resbuf *rbuf);
ACCORE_PORT int            acedRetPoint (const ads_point pt);
ACCORE_PORT int            acedRetStr (const ACHAR *s);
ACCORE_PORT int            acedRetName (const ads_name aname, int type);
ACCORE_PORT int            acedRetInt (int ival);
ACCORE_PORT int            acedRetReal (double rval);
ACCORE_PORT int            acedRetT (void);
ACCORE_PORT int            acedRetNil (void);
ACCORE_PORT int            acedRetVoid (void);
...
#endif
ACCORE_PORT int            acedSSAdd (const ads_name ename, const ads_name sname,
                            ads_name result);
ACCORE_PORT int            acedSSDel (const ads_name ename, const ads_name ss);
ACCORE_PORT int            acedSSMemb (const ads_name ename, const ads_name ss);
ACCORE_PORT int            acedSSName (const ads_name ss, int i, ads_name entres);
ACCORE_PORT int            acedSSNameX (struct resbuf **rbpp, const ads_name ss,
                            int i);
ACCORE_PORT int            acedSSNameXEx (struct resbuf **rbpp, const ads_name ss,
                              int i, unsigned int flags);

ACCORE_PORT int            acedSSGetKwordCallbackPtr(struct resbuf* (**pFunc)(const ACHAR*));
ACCORE_PORT int            acedSSSetKwordCallbackPtr(struct resbuf* (*pFunc)(const ACHAR*));
ACCORE_PORT int            acedSSGetOtherCallbackPtr(struct resbuf* (**pFunc)(const ACHAR*));
ACCORE_PORT int            acedSSSetOtherCallbackPtr(struct resbuf* (*pFunc)(const ACHAR*));
...
ACCORE_PORT int            acedInitGet (int val, const ACHAR *kwl);

ACCORE_PORT int            acedGetSym (const ACHAR *sname, struct resbuf **value);
ACCORE_PORT int            acedPutSym (const ACHAR *sname, struct resbuf *value);
...
ACCORE_PORT int            acedMenuCmd (const ACHAR *str);
ACCORE_PORT int            acedPrompt (const ACHAR *str);
int            acedAlert (const ACHAR *str);

/* Functions used to get user input  */
ACCORE_PORT int            acedGetAngle (const ads_point pt, const ACHAR *prompt, double *result);
ACCORE_PORT int            acedGetCorner (const ads_point pt, const ACHAR *prompt, ads_point result);
ACCORE_PORT int            acedGetDist (const ads_point pt, const ACHAR *prompt, double *result);
ACCORE_PORT int            acedGetOrient (const ads_point pt, const ACHAR *prompt, double *result);
...
ACCORE_PORT int            acedGrDraw (const ads_point from, const ads_point to,
                             int color, int hl);
...
int            acedGetFileD (const ACHAR *title, const ACHAR *defawlt,
                               ...
                               struct resbuf *result);
ACCORE_PORT int            acedGetFileNavDialog (const ACHAR *title, const ACHAR *defawlt,
...
ACCORE_PORT struct resbuf *acdbEntGetX (const ads_name ent, const struct resbuf *args);
ACCORE_PORT struct resbuf *acdbEntGet (const ads_name ent);
int            acdbEntLast (ads_name result);
ACCORE_PORT int            acdbEntNext (const ads_name ent, ads_name result);
ACCORE_PORT int            acdbEntUpd (const ads_name ent);
...
ACCORE_PORT int            acdbRawAngToF (const ACHAR *str, int unit, double *v);
ACCORE_PORT int            acdbDisToF    (const ACHAR *str, int unit, double *v);
