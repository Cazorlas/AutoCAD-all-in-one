...
struct AcGiSentScanLines 
{
    ...
    Adesk::Int8 *    mpPixelBuffer  { nullptr };    // one pointer per scan line
    Adesk::UInt32    mRowBytes      { 0 };          // number of bytes per scan line
    void *           mpImageId      { nullptr };    // image id ptr
    Adesk::Int16     mIsCancelled   { 0 };          // boolean: user cancelled
    Adesk::Int16     mHasFailed     { 0 };          // boolean: request failed
    Adesk::UInt32    mBytes         { 0 };
    IsmRequestStatus mRequestStatus { eOk };
};
...
ACDBCORE2D_PORT Acad::ErrorStatus acgiGetRegenBlockRefStack(AcDbObjectIdArray& blkRefs);
ACDBCORE2D_PORT Acad::ErrorStatus acgiGetCompoundObjStack(AcDbObjectIdArray& comps);
...
public:
    ...
    virtual bool supportsTrueTypeText() const { return false; }
    virtual bool supportsOwnerDraw()    const { return false; }
...
protected:
    ...
    virtual int getTrueTypeTextExtents(ACHAR const *, Charset, double, bool, int,
                                       bool, short, int, double, double,
...
using AcGiContext2 = AcGiContext;       // obsolete alias
...
public:
    ...
    // pNewExtents points to an array of two 3d points
    // Implementation should not modify the points
    // (The pointer arg is missing a const modifier)
    //
