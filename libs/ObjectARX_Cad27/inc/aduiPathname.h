...
// methods
        ...
        //  This routine actually calls ParseRelative() with the process's
        //  current working path
        //
        virtual PathErr Parse( const wchar_t *, bool wildcard_ok,
                               /*internal use only*/bool skipIO);

        // This deprecated legacy final overload is here temporarily to force derived
        // classes to override the new method signature above, instead of this old one.
        // It will be removed eventually.
        // Note that the skipIO arg effectively defaults to false
        //
        virtual PathErr Parse( const wchar_t *name, bool wildcard_ok = false) final
        {
            return this->Parse(name, wildcard_ok, /*skipIO*/false);
        }
        ...
        //
        // Note: Parse() and ParseRelative() ignore surrounding quotes and
        // whitespace from the input string
        // They return PATH_INVALID on invalid input, else PATH_NOERR
        // Input may be invalid because:
        //      Input is a null ptr or empty string
        //      Input contains more than 2 colons (Windows only)
        //      UNC path starts with "\\server" but is missing a "\" after that
        //      Other path problems
        //
        PathErr ParseRelative( const wchar_t * name,
                               const CAdUiPathname* relpath,
                               bool wildcard_ok = false,
                               /*internal use only*/bool skipIO = false);
...
// Split a file path into parent (prefix), filename and extension substrings.
// Return false if file path is invalid or can't be parsed, else return true.
// Ignores quotes and whitespace at either end of the input string.
//
// Extension string has a leading dot character if not empty
// prefix string has a trailing separator if not empty
//
// Note that this API parses the input string via CAduiPathName::ParseRelative(),
// and this may cause the start-in directory to appear in the prefix (parent).
// The parsing may also cause delays if an offline network path is passed in.
// The function fails if there is no filename in the path, as in ".abc"
// It also fails if the path is an existing directory, as in "C:\windows\system32"
//
// Clients should consider using C++'s std::filesystem::path class and its
// corresponding methods parent_path(), stem() and extension()
//
// On Windows, clients can also use the non-standard _wsplitpath_s()
//
ACBASE_PORT bool acutFNSplit(const wchar_t *name, AcString *prebuf,
                             AcString *namebuf, AcString *extbuf);
