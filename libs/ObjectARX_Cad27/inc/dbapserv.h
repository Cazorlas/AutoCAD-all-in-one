...
public:    
    ...
    AcDbHostApplicationServices( bool createGlobals = true )
                : AcDbHostApplicationServices(createGlobals, nullptr) {};
    ACDBCORE2D_PORT ~AcDbHostApplicationServices();

    /// <summary>
    /// Provide additional hints to AcDbHostApplicationServices::findFile on the type of file being searched for
    /// </summary>
    enum FindFileHint
    {
        kDefault = 0,
        kFontFile,              /// Font File, either SHX or TTF
        kCompiledShapeFile,     /// SHX font file
        kTrueTypeFontFile,      /// TTF font file
        kEmbeddedImageFile,
        kXRefDrawing,
        kPatternFile,
        kARXApplication,
        ...
        kPhotometricWebFile,
        kMaterialMapFile,
        kCloudOrProjectFile,    /// Internal use only
        kLinetypeFile,          /// Linetype file (.lin)
        kDrawingStandardsFile,  /// Drawing Standards file (.dws)
        kMLineStyleFile,        /// MLine Style File (.mln)
    };
...
private:
    /// <summary>
    /// This method is called by RealdDWG when a file path is required.  The host application should provide a full path to the file based on the input filename.
    /// The hint parameter is used to provide additional context to the host application as to the filetype being searched for.
    /// </summary>
    /// <param name="pcFullPathOut">Pointer to buffer to receive full path</param>
    /// <param name="nBufferLength">Size of output buffer, in ACHARs</param>
    /// <param name="pcFilename">File name to search for.</param>
    /// <param name="pDb">AcDbDatabase this search is being performed for</param>
    /// <param name="hint">Additional hints to the host on the type of the file being searched for</param>
    /// <returns>Return eOk if the file is found and a full path is in the output buffer.  
    /// Return eFilerError if the file is not found.  
    /// Return eBufferTooSmall if the output buffer is not sufficient for the output path.
    /// </returns>
    /// <remarks>The RealDWG host application defines the search path being used for file searches.
    /// </remarks>
...
public:
    // Clients should call this non-virtual overload, which calls the virtual overload
    // and which does caching and handles arbitrary path lengths
    //
    ...
    AcadInternalServices* acadInternalServices() const
    {
        return mpAcadIntSvcs;
    }
    ...
    // sFileName is in/out arg, name of the substitute font file
    // Returns true if substitute font is found
    ACDBCORE2D_PORT virtual bool getSubstituteFont(AcString &sFileName,
                                                const ACHAR *prompt,
                                                int type, bool fontAlt);

    // Deprecated overload.  Calls the above AcString-based overload
    // pFileName is ptr to ptr to string on heap.  String may be re-allocated
    //
    ACDBCORE2D_PORT virtual bool getSubstituteFont(ACHAR **pFileName,
                                                const ACHAR *prompt,
    ...
    // Called in some cases when an attempt is made to read in a corrupt drawing.
    // id indicates the object which failed to read in, with error code es
    // Return true if your application has handled the error and wishes to continue.
    // If you return true, the readDwgFile() call will likely fail and return an error.
    // If false is returned, the fatalError() method below will be called,
    //
    virtual bool notifyCorruptDrawingFoundOnOpen(AcDbObjectId id, Acad::ErrorStatus es) = 0;

    ACDBCORE2D_PORT static AcDbHostApplicationServices * const & getRefPtr();
...
protected:
    ...
    AcDbHostApplicationServices(AcadInternalServices & rAIS)
            : AcDbHostApplicationServices(false, &rAIS) {};
private:
    ACDBCORE2D_PORT AcDbHostApplicationServices(bool bCreateGlobals,
                AcadInternalServices * pAIS);
    AcadInternalServices * const mpAcadIntSvcs = nullptr;
    // NOTES:
...
{
    static AcDbHostApplicationServices * const & rp = AcDbHostApplicationServices::getRefPtr();
    return rp;
}
