...
namespace Autodesk::AutoCAD::PAL::FileSystem
{
...
// 
{
  ...
  public:
    ...
    // This overload of readString() is deprecated and will be removed
    // It is also final and so cannot be overridden by derived filer classes
    virtual Acad::ErrorStatus   readString(ACHAR** pVal) final;
            Acad::ErrorStatus   readIntoTempString();
            const AcString &    getTempStringRef() const;
    ...
    ACDBCORE2D_PORT Acad::ErrorStatus readFileReference(AcString& reference);


    // <summary>
    //   Structure to hold parameters for writing a file reference.
    // </summary>
    struct WriteFileReferenceParameters
    {
        /// <summary>
        ///  Path to the the external reference.
        /// </summary>
        AcString referencePath;
        /// <summary>
        ///  Pointer to the object that owns the external reference.  Must be DB resident since the handle will be retrieved from the object
        /// </summary>
        const class AcDbObject* pObj = nullptr;
        /// <summary>
        ///  The site ID of the reference.  Used if a single object has multiple references
        /// </summary>
        AcString referenceSiteId;
        /// <summary>
        ///  Indicates whether the file reference is an overlay. Overlay Xrefs are used to reference external files in a way that
        /// prevents them from being included in subsequent references or drawings
        /// </summary>
        bool isOverlay = false;
        
        /// <summary>
        /// Indicates last time when the file reference data has been imported into the DWG.
        /// Only applicable for data types that import data
        /// </summary>
        const class AcDbDate* pLastImportTime = nullptr;

        /// <summary>
        /// Internal use only
        /// </summary>
        const Autodesk::AutoCAD::PAL::FileSystem::FileInfoDescriptor* pFileInfoDescriptor = nullptr;
        /// <summary>
        /// Indicates if this call to writeFileReference should call writeString to write the string to the filer
        /// This should only be set to false if the string is being stored in the DWG using alternative mechanisms, and calling
        /// writeString would break the Drawing format compatibility
        /// </summary>
        bool writeStringToFiler = true;
    };

    /// <summary>
    /// Writes a file reference to the specified object.  
    /// The underlying implementation will call writeString with input path
    /// </summary>
    /// <param name="params">Input parameters</param>
    /// <returns>Returns an Acad::ErrorStatus indicating the success or failure of the operation.</returns>
    ACDBCORE2D_PORT Acad::ErrorStatus writeFileReference(const WriteFileReferenceParameters& params);

    Acad::ErrorStatus writeFileReference(const AcString& referencePath, const AcDbObject* pObj) {
        WriteFileReferenceParameters params;
        params.referencePath = referencePath;
        params.pObj = pObj;
        return writeFileReference(params);
    }
    ...
    //
            ...
            //
            Acad::ErrorStatus   readItem(ACHAR** pVal);         // deprecated
...
// This overload is deprecated and will eventually be removed
...
// deprecated final virtual method, cannot be overridden.
inline Acad::ErrorStatus AcDbDwgFiler::readString(ACHAR** pVal)
{
    if (pVal == nullptr)
        return Acad::eNullPtr;
    auto es = this->readIntoTempString();
    if (es != Acad::eOk)
        return es;
    return acutNewString(getTempStringRef().constPtr(), *pVal);
}

// Helper method for clients that need to assign the input string to other string types
// such as CString or wstring.  First call this and then getTempStringRef()
//
inline Acad::ErrorStatus AcDbDwgFiler::readIntoTempString()
{
    return this->readString(msTemp);
}

// Clients must use or copy this string immediately.  Do not hold on to the reference!
//
inline const AcString & AcDbDwgFiler::getTempStringRef() const
{
    return msTemp;
}
