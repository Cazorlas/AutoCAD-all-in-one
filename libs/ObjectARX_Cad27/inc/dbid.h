...
/// <summary>
/// Deprecated, this function has performance overhead, and it's unnecessary to call isValid() before call functions like acdbOpenObject() and methods like
/// objectClass(), database() and handle(), they will fail gracefully (without crashing) on stale id.
/// To check if the id is stale or not, caller can check if originalDatabase() != nullptr
/// 
...
namespace std
{
    ...
    {
        // hash functor for std containers

        size_t operator()(const AcDbObjectId& val) const noexcept
        {
            return val.asOldId();
        }
    };
