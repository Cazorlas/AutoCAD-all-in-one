...
// call from onidle to consume a background load result; one per call
// returns true if there are tasks pending to be processed
ISMDLLACCESS bool acdbRasterImageAsyncTaskOnIdle();

// call to determine if image is currently loading async
ISMDLLACCESS bool acdbRasterImageIsLoadingAsync(const AcDbObjectId& defId);
...
bool isRasterImageAsyncEnabled(const RasterImageDefImp* pImpDef);
bool isRasterImageAsyncEnabled(const AcDbObjectId& defId);
