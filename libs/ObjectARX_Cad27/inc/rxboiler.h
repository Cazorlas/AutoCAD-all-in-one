...
// Proxy flags must fit in uint16_t.  0xFFFF == std::numeric_limits<uint16_t>::max().  
#define ACRX_STATIC_CHECK_PROXY_FLAGS(flags) static_assert(0 <= (flags) && 0xFFFF > (flags), "Invalid proxy flags value. Must be greater than or equal to 0 and less than 65535 (0xFFFF).")
...
void CLASS_NAME::rxInit() { \
    ACRX_STATIC_CHECK(CLASS_NAME); \
    ACRX_STATIC_CHECK_PROXY_FLAGS(PROXY_FLAGS); \
    ADESK_SUPPRESS_DEPRECATED  \
    gpDesc = newAcRxClass(ACRX_T(#CLASS_NAME), ACRX_T(#PARENT_CLASS), \
