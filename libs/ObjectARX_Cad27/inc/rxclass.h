...
public:
    ...
    AcRxObject* getX(const AcRxClass*);
    ...
    ACBASE_PORT void        getClassVersion(int& dwgVer,
                            int& maintVer) const;
    ACBASE_PORT Adesk::UInt32   proxyFlags() const;
...
protected:
    ...
    ~AcRxClass();
    ACBASE_PORT AcRxObject* subQueryX(const AcRxClass* protocolClass) const override;
private:
...
inline AcRxObject* AcRxClass::getX(const AcRxClass* protocolClass)
{
    auto localProtocol = this->queryX(protocolClass);
    if (localProtocol == nullptr)
       acrx_abort(L"Protocol Extension '%s' not found.",
            protocolClass != nullptr ? protocolClass->name() : L"<null>");
