...
ArxDbgUiDlgClassDict::buildColumns()
{
    ...
    lvc.cx = col1_width;
    // The LV_COLUMN struct is a multipurpose struct, so its pszText member
    // can't be const.  But, the use here is a const use, so it is safe to
    // cast away the const to allow const strings to be used.
    //
    lvc.pszText = const_cast<wchar_t*>(L"Class Name");
    m_classList.InsertColumn(0, &lvc);

    lvc.cx = col2_width;
    lvc.pszText = const_cast<wchar_t*>(L"DXF Name");
    m_classList.InsertColumn(1, &lvc);

    lvc.cx = col3_width;
    lvc.pszText = const_cast<wchar_t*>(L"App Name");
    m_classList.InsertColumn(2, &lvc);

    lvc.cx = col4_width;
    lvc.pszText = const_cast<wchar_t*>(L"Proxy Flags");
    m_classList.InsertColumn(3, &lvc);

    lvc.cx = col5_width;
    lvc.pszText = const_cast<wchar_t*>(L"Birth DWG Version");
    m_classList.InsertColumn(4, &lvc);

    lvc.cx = col6_width;
    lvc.pszText = const_cast<wchar_t*>(L"Birth Maint. Version");
