...
public:
    ...
    static void asdkSimpleToolPalette_CREATESIMPLE(void)
    {
        ...
        try
        {
            ...
            {
                tool.AddRef ();
                // In the Createxxx method uses below, some of the function arguments are non-const
                // but the function uses the argument as if it were const, so it is safe to use
                // a string literal and cast it to non-const.
                //
                AcTcCatalog *pCatalog = tool.CreateStockToolATC(const_cast<wchar_t*>(L"SimpleCatalog"));
                if(pCatalog == NULL)
                    throw _T("Couldn't Create Catalog");

                AcTcPalette* pPalette = tool.CreatePaletteATC(pCatalog, const_cast<wchar_t*>(L"SimplePalette"));
                if(pPalette == NULL)
                    throw _T("Couldn't Create Palette");
                
                AcTcPackage* pPackage = tool.CreateShapeCatalogATC(const_cast<wchar_t*>(L"*AutoCADShapes"));
                if(pPackage == NULL)
                ...
                if(NULL == tool.CreateCommandToolATC(pPalette, const_cast<wchar_t*>(L"Line"), _T("IDB_TOOL1"), tool.GetCmdName()))
