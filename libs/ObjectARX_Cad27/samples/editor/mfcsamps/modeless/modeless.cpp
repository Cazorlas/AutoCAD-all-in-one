...
AsdkDataManager DocDataMgr; //MDI safe
...
void objDisplay(const TCHAR* pWhat, const AcDbObject* pDbObj)
{
    ...
    if (pDbObj && pDbObj->isKindOf(AcDbEntity::desc()) && !DocDataMgr.docData().undoCmd) 
        DocDataMgr.docData().undoBtnAvailable = true;
...
void CEdReactor::commandEnded(const TCHAR *cmd)
{
    ...
    if (DocDataMgr.docData().undoCmd)
        DocDataMgr.docData().undoCmd = false; //undo is finished
}
/////////////////////////////////////////////////////////////////////////////////////
//
