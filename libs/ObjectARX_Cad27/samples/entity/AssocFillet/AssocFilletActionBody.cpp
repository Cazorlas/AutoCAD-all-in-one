...
double AssocFilletActionBody::getRadius(AcString& expressionOut) const
{
    ...
    AcString tmp;
    if (!eOkVerify(getValueParam(kRadiusParamName, value, expressionOut, tmp)))
        return 0.0;
    double radius = 0.0;
...
ErrorStatus AssocFilletActionBody::setRadius(double radius, const AcString& expression)
{
    ASSERT(radius >= 0.0);
    AcString tmp;
    return setValueParam(kRadiusParamName, fabs(radius), expression, L"", tmp, true);
}
...
bool AssocFilletActionBody::isTrimInputEdge(int index, AcString& expressionOut) const
{
    ...
    AcDbEvalVariant value;
    AcString tmp;
    if (!eOkVerify(getValueParam(kTrimInputEdgeParamName, value, expressionOut, tmp, index)))
        return false;
...
ErrorStatus AssocFilletActionBody::setTrimInputEdge(int index, bool yesNo, const AcString& expression)
{
    ASSERT(index == 0 || index == 1);
    AcString tmp;
    ErrorStatus err = setValueParam(kTrimInputEdgeParamName, AcDbEvalVariant((Adesk::Int32)yesNo), expression, L"", tmp, true, index);
    if (!eOkVerify(err))
        return err;
...
ErrorStatus AssocFilletActionBody::computeNewGeometry(bool updateConfigState, AcGeCurve3d* pInputCurveOut[2], AcGeCircArc3d& filletArcOut) 
{
    ...
    AcString tmp;
    const bool isTrimInput[2] = { isTrimInputEdge(0, tmp), isTrimInputEdge(1, tmp), };
    ...
    ErrorStatus err = mFilletConfig.evaluate(updateConfigState, pCurve, getRadius(tmp), isTrimInput, true/*adjustTweakedCurves*/, filletArc);
    if (err == eOk)
    {
    ...
    AcDbObjectIdArray& additionalObjectsToClone) const
{
    for (int i = 0; i < 2; i++)
    {
        ...
        AcDbIdPair pair(inputEdge.entity().leafId(), AcDbObjectId::kNull, false);
        if (!idMap.compute(pair))
