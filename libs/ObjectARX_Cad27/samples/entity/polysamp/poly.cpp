...
//
// To test: APPLOAD asdkpolyui.arx and use the POLY command
    ...
    AcDbIntArray&         /*geomIds*/) const
{
    ...
    if (   osnapMode != AcDb::kOsModeEnd
        ...
        && osnapMode != AcDb::kOsModeCentroid
    ...
    case AcDb::kOsModeCentroid: {
        ...
        for (int i = 0; i < N; i++) {
            sumX += vertexArray[i].x;
            sumY += vertexArray[i].y;
            sumZ += vertexArray[i].z;
        }

        centroid.set(sumX / N, sumY / N, sumZ / N);
        snapPoints.append(centroid);
        break;
    }
