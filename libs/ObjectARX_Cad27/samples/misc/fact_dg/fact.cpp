...
struct func_entry { const wchar_t *func_name; int (*func) (struct resbuf *); };
...
static struct func_entry func_table[] = { {/*MSG0*/L"fact", fact},
                                          {/*MSG0*/L"sqr", squareroot},
                                        };
...
static int dofun()
{
    ...
    if ((val = acedGetFunCode()) < 0 || val >= ELEMENTS(func_table)) {
        acdbFail(/*MSG2*/L"Received nonexistent function code.");
        return RTERROR;
    }
...
fact(struct resbuf *rb)
{
    ...
    if (rb->restype == RTSHORT) {
        ...
        acdbFail(/*MSG3*/L"Argument should be an integer.");
        return RTERROR;
    }

    if (x < 0) {                      /* Check argument range */
        acdbFail(/*MSG4*/L"Argument should be positive.");
        return RTERROR;
    } else if (x > 170) {             /* Avoid floating-point overflow */
        acdbFail(/*MSG5*/L"Argument should be 170 or less.");
        return RTERROR;
    }
...
squareroot(struct resbuf *rb)
{
    ...
    if (rb->restype == RTSHORT) {     /* Save in local variable */
        ...
        acdbFail(/*MSG6*/L"Argument should be a real or integer value.");
        return RTERROR;
    }

    if (x < 0) {                      /* Check argument range */
        acdbFail(/*MSG7*/L"Argument should be positive.");
        return RTERROR;
    }
