...
void asdktest3 () {
    ...
    //----- Process hidden line removal
    AcGePoint3d viewTarget(50, 50,0);
    AcGeVector3d viewDir(0, 0, 1);
    AsdkHlrEngine hlr (viewTarget, viewDir, kEntity | kBlock | kShowAll | kProject | kHonorInternals) ;
    hlr.run (collector) ;
...
void AddEntityToLayer (AsdkHlrCollector &collector, const ACHAR *layerName) {
    //----- Check layer
...
void asdktest4 () {
    ...
    //----- Process hidden line removal
    AcGePoint3d viewTarget(asPnt3d (tarPt));
    AcGeVector3d viewDir(0, -1, 0);
    AsdkHlrEngine hlrFront (
        viewTarget, viewDir,//AcGeVector3d (0, 0, -1),
    ...
    //----- Process hidden line removal
    viewTarget = asPnt3d (tarPt);
    viewDir.x = 1;
    viewDir.y = 0;
    viewDir.z = 0;
    AsdkHlrEngine hlrLeft (
        viewTarget, viewDir,//, AcGeVector3d (1, 0, 0),
        kProject| kEntity | kBlock | kReuse //----- Do not use 'kCleanup'
        | (honorInt == IDYES ? kHonorInternals : 0)
        | (hidLines == IDYES ? kShowAll : 0)
        | (meshSils == IDYES ? kMeshSilhouettes : 0)
    ...
    AsdkHlrEngine hlrTop (
        viewTarget, viewDir,//, AcGeVector3d (0, -1, 0),
        kProject| kEntity | kBlock | kReuse //----- Do not use 'kCleanup'
        | (honorInt == IDYES ? kHonorInternals : 0)
        | (hidLines == IDYES ? kShowAll : 0)
        | (meshSils == IDYES ? kMeshSilhouettes : 0)
