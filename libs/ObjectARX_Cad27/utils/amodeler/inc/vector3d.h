...
public:
    ...
    double   operator [](int index)     const { return m_xyz[index]; }
    double&  operator [](int index)           { return m_xyz[index]; }
    ...
    bool     isParallel (const Vector3d&, double dEpsNorSqrd = epsNorSqrd()) const;
    bool     isParallelNorm(const Vector3d&, double dEpsNorSqrd = epsNorSqrd()) const; // For normalized vectors
    bool     isPerpend  (const Vector3d&, double dEpsNor = epsNor()) const;
